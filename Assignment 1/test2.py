...
def uninf_uniform_cost_search():
    ...
    while(len(fringe) > 0):
        if fringe[0]["current_node"] == dest:
            result = fringe[0]
            break
        elif fringe[0]["current_node"] in visited:
            del fringe[0]
            continue
        else:
            visited.append(fringe[0]["current_node"])
            for i in range(len(map[fringe[0]["current_node"]])):
                if map[fringe[0]["current_node"]][i] > 0:
                    fringe.append({
                        "current_node"      : i,
                        "path"              : []+fringe[0]["path"]+[i],
    ...
    if result:
        ...
        for i in range(len(result["path"]) - 1):
            print places[result["path"][i]] + " to " + places[result["path"][i+1]] + ", " + str(map[result["path"][i]][result["path"][i+1]]) + " km"
    else:
