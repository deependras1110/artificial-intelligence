...
def eval_expression(expression, model):
    if expression.symbol[0]:
        return model[expression.symbol[0]]
    
    else:
        if expression.connective[0] == 'not':
            return not eval_expression(expression.subexpressions[0], model)
        
        elif expression.connective[0] == 'and':
            for subexpression in expression.subexpressions:
                if eval_expression(subexpression, model) == False:
                    return False
            return True
        
        elif expression.connective[0] == 'or':
            for subexpression in expression.subexpressions:
                if eval_expression(subexpression, model) == True:
                    return True
            return False

        elif expression.connective[0] == 'if':
            if eval_expression(expression.subexpressions[1], model) == True:
                return True
            elif eval_expression(expression.subexpressions[0], model) == False:
                return True
            else:
                return False
        
        elif expression.connective[0] == 'iff':
            if eval_expression(expression.subexpressions[1], model) == eval_expression(expression.subexpressions[0], model):
                return True
            else:
                return False
            ...
            values = []
            for subexpression in expression.subexpressions:
                values.append(eval_expression(subexpression, model))
